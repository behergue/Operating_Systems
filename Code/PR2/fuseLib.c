...
//para borrar un fichero (path es su direccion)
//(liberando el espacio que ocupa)
static int my_unlink(const char *path){
    ...
    //decimos que esta libre
    myFileSystem.directory.files[idfich].freeFile = true;
    //hay un fichero menos en el directorio
    myFileSystem.directory.numFiles--;
    //hay un nodo libre mas
    myFileSystem.numFreeNodes++;

    //conseguimos a partir del fichero el i-nodo
    int idnodo = myFileSystem.directory.files[idfich].nodeIdx;

    //el i-nodo ahora esta libre
    myFileSystem.nodes[idnodo]->freeNode = true;

    int bloque;

    //recorremos los bloques que ocupaba el fichero y los vamos liberando
    for(int i = 0; i < MAX_BLOCKS_PER_FILE; i++){
        //bloque guarda el numero de bloque que hay que liberar (si es distinto de 0 entra en el if)
    	if((bloque = myFileSystem.nodes[idnodo]->blocks[i]) != 0){
            //liberamos el bloque en el bitmap
    		myFileSystem.bitMap[bloque] = 0;
            //tambien en la lista de bloques dentro del i-nodo
    		myFileSystem.nodes[idnodo]->blocks[i] = 0;
            //en el superbloque hay un bloque libre mas
    		myFileSystem.superBlock.numOfFreeBlocks++;
    	}
    }

    //liberamos el espacio del i-nodo
    free (myFileSystem.nodes[idnodo]);

    //los updates para que los cambios realizados se reflejen en el fichero
    updateBitmap(&myFileSystem);
    updateDirectory(&myFileSystem);
...
//path es la direccion del fichero donde quieres leer
//en buf devolvemos lo que queremos leer
//size es el tamaño de lo que queremos leer
//offset es donde empezamos a leer
//fuse_file_info fi tiene informacion del fichero (nos sirve para coger el numero de i-nodo)
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi){

    //buffer del tamaño de un bloque donde vamos leyendo los bloques
    char buffer[BLOCK_SIZE_BYTES];
    //variables con lo que llevamos leido y lo que nos falta por leer
    int bytes2Read = size, totalRead = 0;
    //fi->fh nos dice el numero de i-nodo asociado al fichero y nons lo guardamos en node
    NodeStruct *node = myFileSystem.nodes[fi->fh];

    //mensaje de error
    fprintf(stderr, "--->>>my_read: path %s, size %zu, offset %jd, fh %"PRIu64"\n", path, size, (intmax_t)offset, fi->fh);

    //mientras nos queden cosas por leer
    while(bytes2Read) {
        int i;
        int currentBlock, offBlock;
        //currentBlock es el bloque actual que nos toca leer
        currentBlock = node->blocks[offset / BLOCK_SIZE_BYTES];
        //desplazamiento dentro del bloque
        offBlock = offset % BLOCK_SIZE_BYTES;

        //leemos el bloque (entra en el if si lee -1, es decir, si da error)
        if( readBlock(&myFileSystem, currentBlock, &buffer)==-1 ) {
            fprintf(stderr,"Error reading blocks in my_write\n");
            return -EIO;
        }

        //desde el offBlock, si no nos hemos salido del bloque y aun no hemos leido todo
        for(i = offBlock; (i < BLOCK_SIZE_BYTES) && (totalRead < size); i++) {
            //copiamos lo que hemos leido y actualizamos lo que llevamos leido
            buf[totalRead++] = buffer[i] ;
        }

        // Discount the written stuff
        //Actualizamos lo que nos queda por leer
        bytes2Read -= (i - offBlock);
        //Actualizamos el desplazamiento
        offset += (i - offBlock);
    }

    //rellenamos con ceros el resto del espacio (para devolver algo del tamaño size)
    while (totalRead < size){
        buf[totalRead++] = '0';
    }

    //para que se reflejen los cambios en el fichero
    sync();
